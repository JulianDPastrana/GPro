...
#         :returns: log pdf, with shape [...]
#         """
#         mu = self.transform(F[..., :1])
#         sigma = self.transform(F[..., 1:])
#         loc = tf.math.log(mu**2 / (tf.math.sqrt(mu**2 + sigma)))
#         scale = tf.math.log(1 + sigma/mu**2)
#         eps = 1e-6
#         log_unnormalized = -0.5 * tf.square(tf.math.log(Y+eps) - loc) / scale
#         log_normalization = 0.5 * tf.math.log(scale) + 0.5 * np.log(2. * np.pi) + tf.math.log(Y+eps)
#         logp = log_unnormalized - log_normalization
#         return tf.reduce_sum(logp, axis=-1)
...
#         return self.transform(F[..., :1])#tf.math.exp(mu + var / 2)
...
#         return self.transform(F[..., 1:])#first_fact * second_fact
