...
def plot_results(model, X, Y):
    ...
    line = np.linspace(np.ceil(ylim_max), 0.0, num_y_pixels)
    ...
    x, y = np.mgrid[xlim_min:xlim_max:complex(res.shape[0]),
                    ylim_max:ylim_min:complex(res.shape[1])]
