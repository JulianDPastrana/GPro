...
def plot_results(model: Any, X: np.ndarray, Y: np.ndarray, task: int) -> None:
    """
    ...
    dist = model.likelihood.distribution_class(alpha[:, :, task], beta[:, :, task], force_probs_to_zero_outside_support=True)
    for i in range(num_samples):
        predictive_density += dist.prob(line_space).numpy()
    ...
    cs = ax.contourf(x_mesh, y_mesh, np.log10(predictive_density.T))
